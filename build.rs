...
fn main() {
    println!("cargo:rerun-if-changed=data/org.gnome.WgetFrontend.gschema.xml");
    let out_dir = std::path::PathBuf::from(std::env::var("OUT_DIR").unwrap());
    let schema_dir = out_dir.join("wget-schemas/glib-2.0/schemas");
    let _ = std::fs::create_dir_all(&schema_dir);

    let src = std::path::Path::new("data/org.gnome.WgetFrontend.gschema.xml");
    if src.exists() {
        let dst = schema_dir.join("org.gnome.WgetFrontend.gschema.xml");
