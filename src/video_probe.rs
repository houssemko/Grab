//! Page probing + playlist parsing: URL predicates, Drive fallback,
//! extractor-JSON sanitation and collection parsing. Leaf module
//! (media types + url/serde_json + yt_dlp model + gettext +
//! video_tools/video_types leaves): the dialog, worker resolve and
//! engine expansion consume these through the `video` facade.
...
/// Unix timestamp now, seconds. Test-only today; kept as the probe
/// clock alongside `is_expired` for the expiry-safe re-resolve path.
#[allow(dead_code)]
pub fn now_unix() -> i64 {
    SystemTime::now()
...
/// until a fresh URL is stored. Test-only today; kept with `now_unix`.
#[allow(dead_code)]
