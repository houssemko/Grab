...
use crate::video_tools::{MIN_YTDLP_VERSION, ffmpeg_location_dir, parse_yt_dlp_version};
pub use crate::video_tools::{install_ffmpeg, install_ytdlp, resolve_libraries};

/// Facade: probe identity lives in [`video_types`](crate::video_types)
/// now; these re-exports keep every `crate::video::X` path working.
/// (Names used only inside this module or its tests stay imported
/// below without re-export.)
use crate::video_types::{FetchedVideo, HlsSel, codec_preference, filesize_of};
pub use crate::video_types::{
    ProbeResult, VideoFormatOption, VideoInfo, VideoOutcome, is_video_page, preview_fresh,
};
